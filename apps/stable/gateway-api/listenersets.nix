...
let
  ...
  ListenersetsModule = types.submodule (
    { name, ... }:
    {
      options = {
        "namespace" = mkOption {
          type = types.str;
          description = "Namespace for this ListenerSet resource.";
        };
        "listeners" = mkOption {
  ...
  allResources = (mapAttrsToList mkListenerSet cfg."listenersets");
in
{
  options.openkrill.apps."gateway-api" = {
    "listenersets" = mkOption {
      type = types.attrsOf ListenersetsModule;
      default = { };
      description = "ListenerSet CRD instances.";
    };
  };
