...
let
  ...
  KuberokafkasModule = types.submodule (
    { name, ... }:
    {
      options = {
        "namespace" = mkOption {
          type = types.str;
          description = "Namespace for this KuberoKafka resource.";
        };
        "spec" = mkOption {
  ...
  allResources = (mapAttrsToList mkKuberoKafka cfg."kuberokafkas");
in
{
  options.openkrill.apps."kubero" = {
    "kuberokafkas" = mkOption {
      type = types.attrsOf KuberokafkasModule;
      default = { };
      description = "KuberoKafka CRD instances.";
    };
  };
