...
let
  ...
  KuberomailsModule = types.submodule (
    { name, ... }:
    {
      options = {
        "namespace" = mkOption {
          type = types.str;
          description = "Namespace for this KuberoMail resource.";
        };
        "spec" = mkOption {
  ...
  allResources = (mapAttrsToList mkKuberoMail cfg."kuberomails");
in
{
  options.openkrill.apps."kubero" = {
    "kuberomails" = mkOption {
      type = types.attrsOf KuberomailsModule;
      default = { };
      description = "KuberoMail CRD instances.";
    };
  };
