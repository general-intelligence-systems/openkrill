...
let
  ...
  KuberobuildsModule = types.submodule (
    { name, ... }:
    {
      options = {
        "namespace" = mkOption {
          type = types.str;
          description = "Namespace for this KuberoBuild resource.";
        };
        "spec" = mkOption {
  ...
  allResources = (mapAttrsToList mkKuberoBuild cfg."kuberobuilds");
in
{
  options.openkrill.apps."kubero" = {
    "kuberobuilds" = mkOption {
      type = types.attrsOf KuberobuildsModule;
      default = { };
      description = "KuberoBuild CRD instances.";
    };
  };
