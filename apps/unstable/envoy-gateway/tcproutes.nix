...
let
  ...
  TcproutesModule = types.submodule (
    { name, ... }:
    {
      options = {
        "namespace" = mkOption {
          type = types.str;
          description = "Namespace for this TCPRoute resource.";
        };
        "parentRefs" = mkOption {
  ...
  allResources = (mapAttrsToList mkTCPRoute cfg."tcproutes");
in
{
  options.openkrill.apps."envoy-gateway" = {
    "tcproutes" = mkOption {
      type = types.attrsOf TcproutesModule;
      default = { };
      description = "TCPRoute CRD instances.";
    };
  };
