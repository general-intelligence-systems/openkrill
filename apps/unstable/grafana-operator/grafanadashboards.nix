...
let
  ...
  GrafanadashboardsModule = types.submodule (
    { name, ... }:
    {
      options = {
        "namespace" = mkOption {
          type = types.str;
          description = "Namespace for this GrafanaDashboard resource.";
        };
  ...
  mkGrafanaDashboard = name: res: {
    ...
    // optionalAttrs (res."folder" != null) { inherit (res) "folder"; }
    // {
    }
  ...
  allResources = (mapAttrsToList mkGrafanaDashboard cfg."grafanadashboards");
in
{
  options.openkrill.apps."grafana-operator" = {
    "grafanadashboards" = mkOption {
      type = types.attrsOf GrafanadashboardsModule;
      default = { };
      description = "GrafanaDashboard CRD instances.";
    };
  };
