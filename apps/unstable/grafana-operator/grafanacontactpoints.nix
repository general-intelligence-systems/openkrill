...
let
  ...
  GrafanacontactpointsModule = types.submodule (
    { name, ... }:
    {
      options = {
        "namespace" = mkOption {
          type = types.str;
          description = "Namespace for this GrafanaContactPoint resource.";
        };
  ...
  mkGrafanaContactPoint = name: res: {
    ...
    // optionalAttrs res."disableResolveMessage" { inherit (res) "disableResolveMessage"; }
    // {
  ...
  allResources = (mapAttrsToList mkGrafanaContactPoint cfg."grafanacontactpoints");
in
{
  options.openkrill.apps."grafana-operator" = {
    "grafanacontactpoints" = mkOption {
      type = types.attrsOf GrafanacontactpointsModule;
      default = { };
      description = "GrafanaContactPoint CRD instances.";
    };
  };
