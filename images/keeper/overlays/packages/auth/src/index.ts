...
import { proxyAuth } from "./plugins/proxy-auth"; // [overlay] proxy-auth plugin
...
interface AuthConfig {
  ...
  proxyAuthEnabled?: boolean; // [overlay]
...
const createAuth = (config: AuthConfig) => {
  const {
    ...
    proxyAuthEnabled = false, // [overlay]
    polarAccessToken,
    polarMode,
    googleClientId,
    googleClientSecret,
  ...
  // [overlay] Register proxy-auth plugin when enabled.
  if (proxyAuthEnabled) {
    plugins.push(proxyAuth());
  }
