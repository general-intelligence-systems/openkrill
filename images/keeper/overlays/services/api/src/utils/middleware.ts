...
import { auth, database, premiumService, redis, proxyAuthEnabled } from "@/context";
...
// ── [overlay] Proxy auth: resolve userId from Remote-User header ──
// When PROXY_AUTH_ENABLED=true and the reverse proxy (Authelia) sets
// Remote-User, we call the proxy-auth plugin endpoint to find or
// auto-provision the user, then use their userId directly.
const resolveProxyAuthUserId = async (request: Request): Promise<string | null> => {
  if (!proxyAuthEnabled) {
    return null;
  }
  ...
  const remoteEmail = request.headers.get("Remote-Email") ?? undefined;
  const remoteName = request.headers.get("Remote-Name") ?? undefined;

  try {
    const response = await auth.api.proxyAuthSession({
      body: {
        username: remoteUser,
    ...
    // Plugin endpoint not available or failed — fall through to
    // normal auth so we don't lock people out.
    return null;
  }
};
...
const withAuth =
  ...
  async ({ request, params }) => {
    // [overlay] Check proxy auth headers first.
    const proxyUserId = await widelog.time.measure("auth.duration_ms", () =>
      resolveProxyAuthUserId(request),
    );
    if (proxyUserId) {
      widelog.set("auth.method", "proxy");
      await enrichWithUserContext(proxyUserId);
      return handler({ params, request, userId: proxyUserId });
    }
