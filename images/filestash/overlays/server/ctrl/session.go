...
// sessionProxyAuth handles authentication when a trusted reverse proxy
// (e.g. Authelia) has already verified the user and set Remote-User,
// Remote-Email, Remote-Name, and Remote-Groups headers.  No login form
// is ever rendered — we read the headers, run attribute mapping, create
// the session cookie, and redirect.
func sessionProxyAuth(ctx *App, res http.ResponseWriter, req *http.Request) {
	_get := req.URL.Query()
	templateBind := TmplParams(map[string]string{
	...
	// Resolve which backend label to map from query or cookie.
	label := _get.Get("label")
	if label == "" {
		if refCookie, err := req.Cookie("ssoref"); err == nil {
			s := strings.SplitN(refCookie.Value, "::", 2)
			label = s[0]
		}
	}

	// Store label in cookie so subsequent requests can find it.
	if label != "" {
		http.SetCookie(res, applyCookieRules(&http.Cookie{
			Name:   "ssoref",
			Value:  label,
	...
	// Attribute mapping — same logic as the plugin path (Step 3).
	session, err := func() (map[string]string, error) {
		globalMapping := map[string]map[string]interface{}{}
		raw := Config.Get("middleware.attribute_mapping.params").String()
		if err := json.Unmarshal([]byte(raw), &globalMapping); err != nil {
			return nil, err
		}
		out := map[string]string{}
		for k, v := range globalMapping[label] {
			val, err := TmplExec(NewStringFromInterface(v), templateBind)
			if err != nil {
				Log.Debug("session::proxyAuth tmplExec err=%s", err.Error())
			}
			out[k] = val
		}
	...
	if err != nil {
		Log.Warning("session::proxyAuth mapping error: %s", err.Error())
		http.Redirect(res, req,
			WithBase("/?error=Not%20Valid&trace=proxy+auth+mapping+error+-+"+url.QueryEscape(err.Error())),
	...
	if _, err := model.NewBackend(ctx, session); err != nil {
		Log.Info("[auth] status=proxy_failed user=%s ip=%s err=%s",
			req.Header.Get("Remote-User"), ip(req), ferror(err))
		errMsg := "/?error=" + url.QueryEscape(err.Error()) + "&trace=backend+error"
		if IsATranslatedError(err) {
			errMsg = "/?error=" + url.QueryEscape(err.Error())
		}
		http.Redirect(res, req, WithBase(errMsg), http.StatusTemporaryRedirect)
		return
	}
	...
	s, err := json.Marshal(session)
	if err != nil {
		Log.Debug("session::proxyAuth marshal error %+v", session)
		SendErrorResult(res, ErrNotValid)
		return
	}
	obfuscate, err := EncryptString(SECRET_KEY_DERIVATE_FOR_USER, string(s))
	if err != nil {
		Log.Debug("session::proxyAuth encrypt error: %s", err.Error())
		SendErrorResult(res, ErrNotValid)
		return
	}
	http.SetCookie(res, applyCookieRules(&http.Cookie{
	...
	Log.Info("[auth] status=proxy_success user=%s backend=%s::%s ip=%s",
		req.Header.Get("Remote-User"), session["type"], backendID(session), ip(req))
...
func SessionAuthMiddleware(ctx *App, res http.ResponseWriter, req *http.Request) {
	...
	// ── Proxy auth: trust reverse-proxy headers ──────────────────
	// When a reverse proxy (Authelia) has set Remote-User, bypass the
	// entire plugin system and create the session directly from
	// headers + attribute mapping.
	if req.Header.Get("Remote-User") != "" {
		sessionProxyAuth(ctx, res, req)
		return
	}
