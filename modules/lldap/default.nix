# modules/lldap — Light LDAP user management server
...
#
# Bootstrap workflow:
#   1. Pre-create source secret in secret-store namespace with keys:
#      LLDAP_JWT_SECRET, LLDAP_KEY_SEED, LLDAP_LDAP_USER_PASS
#   2. Deploy — LLDAP creates admin user from LLDAP_LDAP_USER_PASS
#   3. Log into the web UI at https://ldap.<domain>
  ...
  options.openkrill.apps.lldap = {
    ...
    baseDn = lib.mkOption {
      ...
      description = ''
        ...
        This determines the LDAP directory structure.
      '';
